...
function translator.init(env)
    ...
    if not schemas and Memory then
        ...
        schemas = {
            liuli_base = Memory(env.engine, Schema("liuli.base")),
            liuli_full = Memory(env.engine, Schema("liuli.words")),
            liuli_trie = core.gen_smart_trie(liuli_rev, "liuli.smart"),
        }
    elseif not schemas then
        schemas = {}
    end
    if not core.base_mem and schemas then
        core.base_mem = schemas.liuli_base
        core.full_mem = schemas.liuli_full
